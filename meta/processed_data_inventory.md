# Processed Data Inventory – Dynamic Business Time-Series Case Studies (Cases A–D)

This inventory lists the **canonical processed artefacts** produced by the preprocessing notebooks for each case study, along with the key metadata needed for downstream modelling (windowing, baselines, diffusion models) and for write-up.

The inventory is restricted to information that is explicitly known from the current workflow context. Where a concrete value is not available, a clear placeholder is used.

---

## 1) Global conventions (applies to every processed dataset)

### 1.1 Canonical schema (final column order)

All processed CSVs follow the same final schema and column order:
...
All processed outputs live under:

* `data/processed/<case_subfolder>/`

### 1.3 Scaling convention

* `value_scaled` is created using `StandardScaler` fitted on **train only**, then applied to the full dataset.
* Per-case scaler parameters are recorded as a small dictionary.

### 1.4 Split convention

* Splits are strictly chronological.
* The canonical processed file is the full dataset (`*_full.csv` or equivalent), and split files are derived from it.

---

## 2) Inventory summary table

| Case | Case study ID (`case_study`) | Native sampling                 | Processed folder                  | Canonical full file          | Split files                                                                               |
| ---- | ---------------------------- | ------------------------------- | --------------------------------- | ---------------------------- | ----------------------------------------------------------------------------------------- |
| A    | `ambient`                    | Hourly                          | `data/processed/ambient/`         | `ambient_processed_full.csv` | `ambient_train.csv`, `ambient_validation.csv`, `ambient_test.csv`                         |
| B    | `nyc_taxi`                   | 30-minute                       | `data/processed/nyc_taxi/`        | `nyc_taxi_full.csv`          | `nyc_taxi_train.csv`, `nyc_taxi_val.csv`, `nyc_taxi_test.csv`                             |
| C    | `cpu`                        | 5-minute                        | `data/processed/cpu_utilisation/` | `cpu_utilisation_full.csv`   | `cpu_utilisation_train.csv`, `cpu_utilisation_validation.csv`, `cpu_utilisation_test.csv` |
| D    | `aiops_kpi`                  | 1-minute (with occasional gaps) | `data/processed/aiops_kpi/`       | `aiops_kpi_full.csv`         | `aiops_kpi_train.csv`, `aiops_kpi_validation.csv`, `aiops_kpi_test.csv`                   |

---

## 3) Case-by-case processed artefact details

### Case A – Ambient temperature (NAB sensor failure)

**Raw source**

* File: `ambient_temperature_system_failure.csv` (NAB `realKnownCause`)
* Labels: `combined_labels.json` (NAB)

**Processed outputs (canonical artefacts)**

* Folder: `data/processed/ambient/`
* Full: `ambient_processed_full.csv`
* Splits:
...
* Native sampling resolution: hourly
* Approximate row count: ~7,267
* Labelled anomalies: 2

**Split design**

* Rule: anomaly-based boundaries (7 days before each labelled anomaly)
* Boundary times:
...
* Recorded parameters: `ambient_scaler_params = {"mean": <ambient_train_mean>, "std": <ambient_train_std>}`

---

### Case B – NYC taxi demand (NAB nyc_taxi)

**Raw source**

* File: `nyc_taxi.csv` (NAB)
* Labels: `combined_labels.json` (NAB)

**Processed outputs (canonical artefacts)**

* Folder: `data/processed/nyc_taxi/`
* Full: `nyc_taxi_full.csv`
* Splits:
...
* Native sampling resolution: 30-minute
* Row count: 10,320
* Labelled anomalies: 5

**Split design**

* Rule: calendar-month boundaries
* Boundary times:

  * `training_end_time = 2014-10-31 23:30:00`
  * `validation_end_time = 2014-12-31 23:30:00`
* Anomaly placement summary (known from notebook outputs):

  * train anomalies: 0
  * validation anomalies: 3
  * test anomalies: 2

**Scaling**

* StandardScaler fit on training split only
* Recorded parameters: `nyc_taxi_scaler_params = {"mean": <nyc_taxi_train_mean>, "std": <nyc_taxi_train_std>}`

---

### Case C – CPU utilisation (ASG misconfiguration, NAB)

**Raw source**

* File: `cpu_utilization_asg_misconfiguration.csv` (NAB `realKnownCause`)
* Labels: `combined_labels.json` (NAB)

**Processed outputs (canonical artefacts)**

* Folder: `data/processed/cpu_utilisation/`
* Full: `cpu_utilisation_full.csv`
* Splits:
...
* Native sampling resolution: 5-minute
* Labelled anomalies: 2
* Total row count: <cpu_total_rows>

**Split design**

* Rule: regime-informed boundary selection supported by a baseline reference window
* Key split-validity constraint:

  * minimum test duration enforced (2 days under 5-minute sampling)
  * if needed, `validation_end_time` adjusted earlier to preserve test window

**Scaling**

* StandardScaler fit on training split only
* Recorded parameters: `cpu_scaler_params = {"mean": <cpu_train_mean>, "std": <cpu_train_std>}`

---

### Case D – AIOps KPI (KPI ID `da403e4e3f87c9e0`)

**Raw source**

* File: stacked AIOps training file `train.csv` (multiple KPIs)
* Selection: filtered to KPI ID `da403e4e3f87c9e0`
* Raw fields prior to standardisation: Unix timestamp (seconds), KPI value, binary label, KPI ID

**Processed outputs (canonical artefacts)**

* Folder: `data/processed/aiops_kpi/`
* Full: `aiops_kpi_full.csv`
* Splits:
...
* Native sampling resolution: 1-minute
* Occasional larger gaps exist (treated as missing periods in the time axis; not imputed during preprocessing)
* Total rows: 129,035
* Total anomalies (labelled): 7,666
* Anomaly ratio: ≈ 0.059410 (≈ 5.94%)

**KPI selection rationale (evidence summary)**

* Selected via KPI-level screening using total points, anomaly points, and anomaly ratio.
* The selected KPI provides a long series with meaningful but imbalanced anomalies and visible variation in anomaly density across time segments.
* Segment-level anomaly ratios (early/middle/late thirds):

  * early ≈ 0.008835
  * middle ≈ 0.101530
  * late ≈ 0.067865

**Split design (final chosen boundaries)**
...
**Split summary (known from notebook outputs)**
...
* Train:
  ...
  * anomalies: 563
  * proportion: 49.598%
  * time span: 2017-04-30 16:00:00 → 2017-06-15 23:59:00
* Validation:

  * rows: 17,268
  * anomalies: 97
  * proportion: 13.382%
  * time span: 2017-06-16 00:00:00 → 2017-06-27 23:59:00
* Test:

  * rows: 47,768
  * anomalies: 7,006
  * proportion: 37.019%
  * time span: 2017-06-28 00:00:00 → 2017-07-31 04:29:00
...
## 4) Modelling readiness notes (inventory-level)

* All four cases now have canonical processed artefacts saved to disk under a consistent schema.
* For Case D (AIOps KPI), gaps are preserved as missing periods in the time axis; later window creation must enforce contiguity (exclude windows that cross non-contiguous time intervals).
* Synthetic anomaly injection (if used later) should be performed on **copies** of these processed datasets and should not modify the canonical processed artefacts listed in this inventory.
